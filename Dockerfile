...
COPY ../music/src /app/src

# 将pom.xml文件，拷贝到工作目录下
COPY ../music/settings.xml pom.xml /app/
...
ENV DATABASE_NAME zhiyuan-music
...
# 将构建产物jar包拷贝到运行时目录中
COPY --from=build /app/target/zhiyuan-music-0.0.1.jar .

# 暴露端口
EXPOSE 80

# 执行启动命令
CMD ["java", "-jar", "/app/zhiyuan-music-0.0.1.jar"]